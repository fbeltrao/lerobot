...
def extract_from_dataset_version_3_1(root: Path) -> LeRobotDatasetTags:
    ...
    return {
        ...
        "tasks": list_to_tag_value(
            set(task for episode_tasks in dataset_metadata.episodes["tasks"] for task in episode_tasks)
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Extracts datasets tags from a local lerobot dataset.")
